...
void TwoSpiralDataSet::load()
{
	...
	this->inputSet.resize(dataNum, 2);
	this->teachSet.resize(dataNum, 1);
	...
	for (int i = 0; i < POINTS; ++i)
	{
		...
		this->inputSet.row(2 * i) << x + MAXDIAMETER / 2.0, y + MAXDIAMETER / 2.0;
		this->inputSet.row(2 * i + 1) << -x + MAXDIAMETER / 2.0, -y + MAXDIAMETER / 2.0;
		this->teachSet.row(2 * i) << 1;
		this->teachSet.row(2 * i + 1) << 0;
	}
